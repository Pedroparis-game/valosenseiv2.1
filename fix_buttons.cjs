...
// Add state for modal
if (!content.includes('const [selectedInsight, setSelectedInsight] = useState')) {
...
const oldBtn = `<button className={\`px-4 py-2 text-[10px] font-bold uppercase tracking-widest transition-colors \${isHero ? 'bg-brand-red text-white hover:bg-brand-red/80' : 'bg-brand-gray/10 text-brand-light hover:bg-brand-gray/30'}\`}>
...
// Add modal JSX at the end before final closing div
const modalJsx = `
      ...
      <AnimatePresence>
        {selectedInsight && (
          <motion.div
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-brand-darker/90 backdrop-blur-sm"
                 ...
                 <div className="space-y-4">
                    ...
                    <div className="bg-brand-darker/50 border border-brand-gray/10 p-6">
                       <ul className="space-y-4">
                         {selectedInsight.checklist.map((item: string, i: number) => (
